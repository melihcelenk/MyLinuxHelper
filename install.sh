...
_install_do() {
	...
	for pkg in "$@"; do
		...
		case "$manager" in
		apt | apt-get) dpkg -s "$pkg" >/dev/null 2>&1 || {
			echo "Error: '$pkg' not marked installed by dpkg."
			return 1
		} ;;
		dnf | yum | zypper) rpm -q "$pkg" >/dev/null 2>&1 || {
			echo "Error: '$pkg' not marked installed by rpm."
			return 1
		} ;;
		pacman) pacman -Qi "$pkg" >/dev/null 2>&1 || {
			echo "Error: '$pkg' not marked installed by pacman."
			return 1
		} ;;
		apk) apk info -e "$pkg" >/dev/null 2>&1 || {
			echo "Error: '$pkg' not marked installed by apk."
			return 1
		} ;;
		esac
...
EOF
	...
	local manager
	manager="$(_install_detect_manager)"
	[ -z "$manager" ] && {
